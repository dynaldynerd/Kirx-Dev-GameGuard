...
| `WorldServerUtil.cpp` | `GetItemEquipCivil` | `s_ptblItemData[nTableCode]` | `_base_fld *` | Uses raw offsets like `Record[3].m_strCode[12/8/4/56/60]`, `Record[6].m_strCode[16]`, `Record[2]`, or `reinterpret_cast<char *>(&Record[3])` depending on table code. |
| `WorldServerUtil.cpp` | `IsSellItem` | `s_ptblItemData[nTableCode]` | `_base_fld *` | Uses many raw offsets per table code (e.g. `Record[6].m_strCode[32]`, `Record[11].m_dwIndex`, `Record[5].m_strCode[32]`, `Record[7].m_strCode[60]`, `Record[4].m_strCode[56]`). |
