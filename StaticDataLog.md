...
## R3 map-path defaults (2026-02-14)
- `".\\Map\\"` (IDA `InitCore` default for `stState.MapPath`)
- `".\\R3Engine.ini"` (IDA `InitCore` source ini for path defaults)
...
## Newly logged (send-packet caller batch 2026-02-14)
- Static string `"Create Master >> race:%d, name:%s, stone:%d"` (`CHolyStoneSystem::CheckHolyMaster`)
- Static string `">> Change Schedule : %d -> HS_SCENE_BATTLE_END_WAIT_TIME, PlusSecTime:%d, controlSecTime:%d"` (`CHolyStoneSystem::CheckHolyMaster`)
- Static string `"WIN : %s (%s), Destroy(%s)"` (`CHolyStoneSystem::CheckHolyMaster`)
- Static data literal `"2e"` used as 2-byte type payload (`CHolyStoneSystem::CheckHolyMaster`, web report send)
- Packet type bytes `{55, 4}` (`GMCallMgr::SendResponseGMCall`)
- Packet type bytes `{55, 5}` (`GMCallMgr::SendResponseGMList`)
- Packet type bytes `{55, 6}` (`GMCallMgr::SendResponseAcceptResult`)
- Static string `"(%u)CNormalGuildBattle::Start : byColor(%u)\\n"` (`GUILD_BATTLE::CNormalGuildBattle::Start`, `GUILD_BATTLE::CNormalGuildBattle::ReStart`)
- Static string `"CNormalGuildBattle::Start( %u, %u ) : (%d) %s Guild : (%d) %s race(%d) Start!"` (`GUILD_BATTLE::CNormalGuildBattle::Start`, `GUILD_BATTLE::CNormalGuildBattle::ReStart`)
- Static string `"CNormalGuildBattle::Start( %u, %u ) : CGuildBattleSchedulePool::Instance()->GetRef(%u) NULL!"` (`GUILD_BATTLE::CNormalGuildBattle::Start`, `GUILD_BATTLE::CNormalGuildBattle::ReStart`)
- Static string `"CNormalGuildBattle::SendKillInform() : CGuildBattleSchedulePool::Instance()->GetRef(%u) NULL!"` (`GUILD_BATTLE::CNormalGuildBattle::SendGoalMsg`)
- Static string `"Cheat"` (`CGuildBattleController::CheatTakeStone`, `CGuildBattleController::CheatGetStone`)
- Packet type bytes `{27, 57}` (`GUILD_BATTLE::CNormalGuildBattleManager::Join`)
- Packet type bytes `{27, 69}` (`CGuildBattleController::CheatRegenStone`)
- Packet type bytes `{27, 72}` (`CGuildBattleController::CheatTakeStone`, `CGuildBattleController::CheatGetStone`)
- Packet type bytes `{27, 84}` (`GUILD_BATTLE::CNormalGuildBattle::ReStart`)
- Packet type bytes `{27, 68}` (`GUILD_BATTLE::CNormalGuildBattle::Start`)
- Packet type bytes `{27, 88}` (`GUILD_BATTLE::CNormalGuildBattle::NotifyBeforeStart`)
- Packet type bytes `{27, 89}` (`GUILD_BATTLE::CNormalGuildBattle::NotifyBattleResult`)
- Packet type bytes `{27, 74}` (`GUILD_BATTLE::CNormalGuildBattle::SendGoalMsg`)
- Packet type bytes `{51, 17}` (`GUILD_BATTLE::CNormalGuildBattle::SendWebBattleStartInfo`, `GUILD_BATTLE::CNormalGuildBattle::SendWebBattleEndInfo`)
- Packet type bytes `{51, 18}` (`GUILD_BATTLE::CNormalGuildBattle::GuildBattleResultLogNotifyWeb`)
- Static string `"_AntiCpSvr_MakeReqMsg n(%d) Err(%u) Error!"` (`HACKSHEILD_PARAM_ANTICP::CheckClient`)
- Static string `"HACKSHEILD_PARAM_ANTICP::Kick n(%d) Reason ( %s ), ( %u ) Kick!"` (`HACKSHEILD_PARAM_ANTICP::Kick`)
- Static string `"HackShield Kick"` (`HACKSHEILD_PARAM_ANTICP::Kick`)
- Static strings `"KICK_REASON_CRC_ACK_DELAY"`, `"KICK_REASON_FIRST_GUID_INVALID"`, `"KICK_REASON_CRC_ACK_INVALID"`, `"KICK_REASON_NOT_HACKSHEILD_CLIENT"`, `"What ??? "` (`_KickReason`, hackshield flow)
- Packet type bytes `{98, 4}` (`HACKSHEILD_PARAM_ANTICP::CheckClient`)
- Packet type bytes `{1, 2}` (`HACKSHEILD_PARAM_ANTICP::Kick`)
- Packet type bytes `{98, 2}` (`HACKSHEILD_PARAM_ANTICP::OnRecvSession_ServerCheckSum_Request`)
- Packet type bytes `{98, 6}` (`HACKSHEILD_PARAM_ANTICP::OnRecvSession_ClientCheckSum_Response`)

## Newly logged (mapoperation-ctor bootstrap 2026-02-14)
- Static string `"Path"` (`InitR3Engine`, INI section for map path)
- Static string `"MapPath"` (`InitR3Engine`, INI key for map path)
- Static string `"SystemPath"` (`InitR3Engine`, INI key for system path)
- Static string `"EntityPath"` (`InitR3Engine`, INI key for entity path)
- Static string `"EffectPath"` (`InitR3Engine`, INI key for effect path)
- Static string `"ScreenShotsPath"` (`InitR3Engine`, INI key for screenshot path)
- Static string `"SoundPath"` (`InitR3Engine`, INI key for sound path)
- Static string `".\\System\\"` (`InitR3Engine`, default system path)
- Static string `".\\Map\\Entity\\"` (`InitR3Engine`, default entity path)
- Static string `".\\Effect\\"` (`InitR3Engine`, default effect path)
- Static string `".\\ScreenShots\\"` (`InitR3Engine`, default screenshot path)
- Static string `".\\Snd\\"` (`InitR3Engine`, default sound path)

## Newly logged (InitR3Engine old-source alignment 2026-02-14)
- Static strings `"RenderState"`, `"Sound"`, `"SampleCharacter"` (`InitR3Engine`, INI sections)
- Static keys `"ScreenXSize"`, `"ScreenYSize"`, `"RenderBits"`, `"ZbufferBits"`, `"bFullScreen"` (`InitR3Engine`)
- Static keys `"TextureDetail"`, `"DynamicLight"`, `"ShadowDetail"`, `"bDetailTexture"`, `"bMouseAccelation"`, `"SeeDistance"` (`InitR3Engine`)
- Static keys `"MusicVol"`, `"SoundVol"`, `"Gamma"`, `"bStartCamera"` (`InitR3Engine`)
- Static keys `"CharPath"`, `"CharTexturePath"`, `"MeshName"`, `"BoneName"`, `"StopAni"`, `"RunAni"`, `"SwordAni"`, `"SwordMesh"`, `"SwordTexturePath"` (`InitR3Engine`)
- Static defaults `"1024"`, `"768"`, `"32"`, `"24"`, `"FALSE"`, `"2"`, `"1"`, `"TRUE"` (`InitR3Engine`)
- Static defaults `".\\Character\\"`, `".\\Character\\tex\\"`, `"SNWG_001.msh"`, `"bone_b.bn"`, `"Stop.ani"`, `"Run_Loop.ani"`, `""` (`InitR3Engine`)
- Static data `kSeeDistanceTable = {0.6f, 0.8f, 1.0f}` (`InitR3Engine`)

## Newly logged (holy-schedule-loader alignment 2026-02-14)
- Static string `"total_schedule_count"` (`CHolyStoneSystemDataMgr::LoadSceduleData`, INI section)
- Static string `"schedule_count"` (`CHolyStoneSystemDataMgr::LoadSceduleData`, INI key)
