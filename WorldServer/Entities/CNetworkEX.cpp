...
{
  // Yorozuya fix (non-IDA parity): guard client index on client line.
  if (n >= MAX_PLAYER)
  {
    return false;
  }
    ...
    case 34:
    {
      ...
      switch (subType)
      {
        ...
        case 207:
          ...
          break;
        default:
          result = subType != 210 || DarkHoleAnswerReenterRequest(n, pMsg);
          break;
      }
      break;
    }
...
bool CNetworkEX::Apex_R(int n, unsigned __int16 wSize, char *pBuf)
{
...
bool CNetworkEX::Apex_T(int n, unsigned __int16 wSize, char *pBuf)
{
  UNREFERENCED_PARAMETER(n);
  UNREFERENCED_PARAMETER(wSize);
  UNREFERENCED_PARAMETER(pBuf);
...
bool CNetworkEX::SelectClassRequest(unsigned int n, char *pBuf)
{
  ...
  if (selClassIndex < recordNum)
  {
    ...
    if (selectRewardItem == 255 || selectRewardItem < 9)
    {
...
bool CNetworkEX::AttackForceRequest(unsigned int n, char *pBuf)
{
  ...
  auto *target = reinterpret_cast<CCharacter *>(
    g_Main.GetObjectA(0, static_cast<unsigned __int8>(request->byID), request->wIndex));
  if (target
      || (static_cast<unsigned __int8>(request->byID) == 255 && request->wIndex == 65535))
  {
    float areaPos[3]{};
...
bool CNetworkEX::AttackTestRequest(unsigned int n, char *pBuf)
{
  ...
  const unsigned __int8 effectCode = static_cast<unsigned __int8>(request->byEffectCode);
  if (effectCode == 255 || effectCode < 4)
  {
    if (effectCode >= 4u
        || g_Main.m_tblEffectData[effectCode].GetRecord(static_cast<unsigned __int8>(request->byEffectIndex)))
    {
      ...
      if (weaponPart == 255 || weaponPart < 2)
      {
        return true;
      }
...
bool CNetworkEX::CombineExItemAcceptRequest(unsigned int n, char *pBuf)
{
  ...
  if (request->SelectItemBuff.bySelectNum < 24)
  {
    if (request->byDlgType == 255 || request->byDlgType == 1 || !request->byDlgType)
    {
...
bool CNetworkEX::UseRecallTeleportItemRequest(unsigned int n, _use_recall_teleport_item_request_clzo *request)
{
  ...
  if (item->byStorageCode)
  {
    player->SendMsg_UsePotionResult(25, item->wItemSerial, 0);
  }

  char targetName[17]{};
  strncpy_s(targetName, sizeof(targetName), request->wszRecallName, 16);
  CNationSettingManager *manager = CTSingleton<CNationSettingManager>::Instance();
  if (manager->IsNormalString(targetName))
  {
    ...
    else
    {
      ...
      recall->SendRecallReqeustResult(17, player);
    }
    return true;
  }
...
bool CNetworkEX::BillingLineAnalysis(int n, _MSG_HEADER *pMsgHeader, char *pMsg)
{
  ...
  return pMsgHeader->m_byType[1] != 'Z'
...
{
  if (request->idLocal.wIndex > 2532)
  {
    return false;
  }
...
bool CNetworkEX::PartyReqBlockReport(int n, const _party_req_block_report_clzo *request)
{
  CPlayer *player = &g_Player[n];
  if (player->m_bOper)
  {
...
bool CNetworkEX::WhisperBlockReport(int n, const _whisper_block_report_clzo *request)
{
  CPlayer *player = &g_Player[n];
  if (player->m_bOper)
  {
...
bool CNetworkEX::TradeBlockReport(int n, const _trade_block_report_clzo *request)
{
  CPlayer *player = &g_Player[n];
  if (player->m_bOper)
  {
...
bool CNetworkEX::GuildBattleBlockReport(int n, const _guild_battle_block_report_clzo *request)
{
  CPlayer *player = &g_Player[n];
  if (player->m_bOper)
  {
...
bool CNetworkEX::TrunkHintAnswerRequest(int n, const _trunk_hint_answer_request_clzo *request)
{
  CPlayer *player = &g_Player[n];
  if (!player->m_bOper || player->m_pmTrd.bDTradeMode)
  {
...
CUserDB *userDb = &g_UserDB[n];
  ...
  g_Main.PushDQSData(-1, nullptr, 153, reinterpret_cast<char *>(&query), size);
  return true;
}
...
bool CNetworkEX::PartyJoinInvitationAnswer(int n, _party_join_invitation_answer_clzo *request)
{
  CPlayer *player = &g_Player[n];
  if (!player->m_bOper || player->m_pmTrd.bDTradeMode || player->m_bCorpse)
  {
...
bool CNetworkEX::PartyJoinApplicatiohAnswer(int n, _party_join_application_answer_clzo *request)
{
  CPlayer *player = &g_Player[n];
  if (!player->m_bOper || player->m_pmTrd.bDTradeMode || player->m_bCorpse)
  {
...
bool CNetworkEX::PartyLockRequest(int n, const _party_lock_request_clzo *request)
{
  CPlayer *player = &g_Player[n];
  if (!player->m_bOper || player->m_pmTrd.bDTradeMode || player->m_bCorpse)
  {
...
bool CNetworkEX::AwayPartyInvitation(int n, const _away_party_invitation_request_clzo *request)
{
  CPlayer *player = &g_Player[n];
  char inviteeName[17]{};
  if (!player->m_bOper || player->m_pmTrd.bDTradeMode || player->m_bCorpse)
...
bool CNetworkEX::SkillRecallTeleportRequest(int n, char *pBuf)
{
  ...
  if (skillIndex < recordNum)
  {
    ...
    else
    {
      ...
      controller->SendRecallReqeustResult(17, player);
    }
    return true;
  }
...
bool CNetworkEX::DTradeAnswerRequest(int n, _d_trade_answer_request_clzo *request)
{
  CPlayer *player = &g_Player[n];
  if (!player->m_bOper || player->m_pmTrd.bDTradeMode || player->m_bCorpse)
  {
...
bool CNetworkEX::QuestSelectRewardReport(int n, char *pBuf)
{
  ...
  if (questSlot < 30)
  {
    ...
    if (rewardSlot == 255 || rewardSlot < 6)
    {
      if (linkIndex == 255 || linkIndex < 5)
      {
        if (rewardSlot == 255 && linkIndex == 255)
        {
          const char *charName = player->m_Param.GetCharNameA();
          m_LogFile.Write(
...
bool CNetworkEX::GuildEstablishRequest(int n, const _guild_establish_request_clzo *request)
{
  CPlayer *player = &g_Player[n];
  if (!player->m_bOper || player->m_pmTrd.bDTradeMode)
  {
...
bool CNetworkEX::GuildJoinApplyRequest(int n, const _guild_join_apply_request_clzo *request)
{
  CPlayer *player = &g_Player[n];
  if (player->m_bOper)
  {
...
bool CNetworkEX::OpenControlInform(unsigned int n, const _open_control_inform_acwr *request)
{
  (void)n;
  (void)request;
...
bool CNetworkEX::UserBlockResult(unsigned int n, const _user_block_result_acwr *request)
{
  (void)n;
