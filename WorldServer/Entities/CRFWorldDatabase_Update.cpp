  ...
  if (m_hStmtSelect || ReConnectDataBase())
  {
    ...
    if (!ret || ret == SQL_SUCCESS_WITH_INFO)
    {
      ...
      if (!ret || ret == SQL_SUCCESS_WITH_INFO)
      {
        ...
        {
          ret = SQLGetData(m_hStmtSelect, ++column, SQL_C_LONG, &pInven->list[index], 0, &indicator);
          ret = SQLGetData(m_hStmtSelect, ++column, SQL_C_TINYINT, &pInven->list[index].byNum, 0, &indicator);
          if (ret && ret != SQL_SUCCESS_WITH_INFO)
          {
            unsigned __int8 result = 0;
            if (ret == SQL_NO_DATA)
