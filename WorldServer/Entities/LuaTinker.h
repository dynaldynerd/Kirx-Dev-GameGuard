...
namespace lua_tinker
{
	...
	{ 
		...
		{ 
			...
			meta_push(L, class_name<typename class_type<T>::type>::name());
	...
	{
		int _var;
		explicit mem_var(V *val) : _var((int)(intptr_t)val) {}

		void get(lua_State *L)
		{
			T *obj = read<T *>(L, 1);
			push<if_<is_obj<V>::value, V &, V>::type>(
				L,
				*reinterpret_cast<V *>(reinterpret_cast<char *>(obj) + _var));
		}
		...
		void set(lua_State *L)
		{
			V value = read<V>(L, 3);
			T *obj = read<T *>(L, 1);
			*reinterpret_cast<V *>(reinterpret_cast<char *>(obj) + _var) = value;
		}
	};
	...
	template<typename T1, typename T2, typename T3>
	struct constructor<T1, T2, T3, void>
	{
		template<typename T>
		static void invoke(lua_State *L)
		{
	...
	template<typename T1, typename T2>
	struct constructor<T1, T2, void, void>
	{
		template<typename T>
		static void invoke(lua_State *L)
		{
	...
	template<typename T1>
	struct constructor<T1, void, void, void>
	{
		template<typename T>
		static void invoke(lua_State *L)
		{
	...
	template<>
	struct constructor<void, void, void, void>
	{
		template<typename T>
		static void invoke(lua_State *L)
		{
	...
	struct creator
	{
		...
		{
			Constructor::template invoke<T>(L);
	...
	void class_con(lua_State* L, F)
	{
		...
		if(lua_type(L, -1) == LUA_TTABLE)
		{
			...
			lua_pushcclosure(L, creator<T>::template invoke<F>, 0);
			lua_rawset(L, -3);
			lua_setmetatable(L, -2);
		}
		lua_settop(L, -2);
	...
	void class_mem(lua_State *L, const char *name, VAR *val)
	{
		meta_push(L, class_name<T>::name());
		if (lua_type(L, -1) == LUA_TTABLE)
		{
